...
API_URL	기본	Grid_20150406000000000217_1	OpenAPI 서비스 URL
START_INDEX	기본	1	요청시작위치
END_INDEX	기본	5	요청종료위치
EXAMIN_DE	필수	20140818	가격조사 일자
FRMPRD_CATGORY_CD	선택	200	부류코드
PRDLST_CD	선택	241	품목 코드
SPCIES_CD	선택	03	품종 코드
AREA_CD	선택	2100	지역 코드
MRKT_CD	선택	0210022	시장 코드
"""
...
base_url = (
    f"http://211.237.50.150:7080/openapi/{api_key}/json/Grid_20150406000000000217_1/"
)
START_INDEX = 1
END_INDEX = 1000
...
while current_date <= end_date:
    ...
    try:
        ...
        if (
            "Grid_20150406000000000217_1" in data
            and "row" in data["Grid_20150406000000000217_1"]
        ):
            rows = data["Grid_20150406000000000217_1"]["row"]
            if len(rows) > 0:
                all_data.extend(rows)
            else:
                empty_row = get_empty_data(examin_de)
                all_data.append(empty_row)
            if data["Grid_20150406000000000217_1"]["totalCnt"] > END_INDEX:
                print(
                    f"Total count is greater than end index for {examin_de}: {data['Grid_20150406000000000217_1']['totalCnt']}"
                )
        else:
...
if all_data:
    ...
    output_filename = f"./data/raw/domae/domae_{start_date_str}_{end_date_str}.csv"
