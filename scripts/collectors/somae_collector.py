...
# .env 호환: RETAIL_API_*, PRODUCT_CODE, API_KEY
...
class SomaeCollector(BaseCollector):
    """소매 데이터 수집기 - 날짜 배치 병렬 처리"""

    def __init__(
        self,
        start_date: date,
        end_date: date,
        output_dir: str = "./data/raw/somae",
        max_workers: int = 10,
        seed: int = 42,
    ):
    ...
    def collect(self) -> pd.DataFrame:
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in tqdm(as_completed(futures), total=len(futures), desc="somae", unit="day", unit_scale=False):
                rows = future.result()
